...
class AIReleaseAnalyzer:
    ...
    def generate_release_notes(self, analysis: Dict, version: str) -> str:
        ...
        # Fallback template
        github_repo = os.environ.get('GITHUB_REPOSITORY', 'oatakan/ansible-role-rhel_upgrade')
        github_owner = github_repo.split('/')[0] if '/' in github_repo else 'oatakan'
        galaxy_role_name = 'rhel_upgrade'
...
```bash
ansible-galaxy install {github_owner}.{galaxy_role_name},{version}
```
...
**Full Changelog**: https://github.com/{github_repo}/blob/main/CHANGELOG.md
"""
        return notes
