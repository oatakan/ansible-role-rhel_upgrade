    ...
    def basic_analysis(self) -> Dict:
        ...
        return {
            ...
            'testing_recommendations': [
                'Run static checks: yamllint + ansible-lint',
                'Run syntax check: ansible-playbook -i tests/inventory tests/test.yml --syntax-check',
                'Run Molecule VM scenario if available: molecule test -s vagrant',
                'Test idempotency (run role twice on a VM)',
                'Validate upgrade + kernel/reboot behavior on target platforms'
            ],
            'code_quality_notes': [
            ...
            'compatibility_notes': [
                'This role performs OS package upgrades; container-based tests may not reflect real behavior'
            ],
            'documentation_needs': [
