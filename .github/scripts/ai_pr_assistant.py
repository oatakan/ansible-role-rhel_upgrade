...
2. **Playbook syntax-check (smoke)**
    ```bash
    ansible-playbook -i tests/inventory tests/test.yml --syntax-check
    ```

### VM Integration Tests (recommended for upgrade roles)
1. **Molecule Vagrant scenario**
    ```bash
    molecule test -s vagrant
    ```

    Notes:
    - This role may upgrade kernels and optionally reboot; VM tests should either disable reboot (`update_reboot_kernel: false`) or ensure the environment can tolerate reboots.
...
- [ ] RHEL 9  
- [ ] Rocky Linux 9
- [ ] CentOS (EOL vault fix path if applicable)
